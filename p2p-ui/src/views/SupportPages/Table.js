...
import { CTable, CTableHead, CTableRow, CTableHeaderCell, CTableBody, CTableDataCell, CPagination, CPaginationItem, CFormInput, CFormSelect, CCol, CRow, CTooltip, CFormSwitch } from '@coreui/react';
import { FaSort, FaSortUp, FaSortDown, FaFileExcel, FaFileCsv, FaFilePdf, FaClipboardList, FaPencilAlt, FaTrashAlt, FaCheckCircle, FaTimesCircle } from 'react-icons/fa'; // Import icons
...
const Table = ({ data, columns, actions, downloadName }) => {
                    ...
                    <CTableDataCell className='text-center align-middle'>
                      <div className="d-flex justify-content-center gap-2">
                        {actions.map(({ label, onClick }) => (
                          <CTooltip key={label} content={(label === 'Enable' && item.activeStatus === 1) ? 'Enable' : (label === 'Enable' && item.activeStatus === 0) ? 'Disable' : label}>
                            <span className="me-2 cursor-pointer">
                              ...
                              {label === 'Enable' && (
                                <CFormSwitch color={item.activeStatus === 1 ? 'green' : 'red'} checked={item.activeStatus === 0} onChange={() => onClick({ ...item, activeStatus: item.activeStatus === 1 ? 0 : 1 })} />
                              )}
                            </span>
                          </CTooltip>
                        ))}
