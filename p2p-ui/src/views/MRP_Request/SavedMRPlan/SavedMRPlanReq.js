...
import PrimaryDetails from '../AddMRPlan/PrimaryDetails';
...
import ProductDetails from '../AddMRPlan/ProductDetails';
import axios from 'axios';
import MRPlanPreview from '../AddMRPlan/MRPlanPreview';
import { FaRegQuestionCircle } from 'react-icons/fa';
import { formatDate } from '../../../utils/DateFormatter';

const SavedMRPlanReq = () => {
  const { mrpId : id } = useParams();
...
// for Remarks
  ...
  const fetchMRPDetails = async () => {
    setLoading(true);
    try{
      const response = await axios.get(`${BASE_URL_P2P}mrPlan/getSavedMrpDetails/${id}`, {
        headers: {
          Authorization: `Bearer ${jwtToken}`,
          "Content-Type": 'application/json'
        }
      });
      const mrpDetData = response.data.map(mrpDet => ({
        ...mrpDet,
        officeLocId: { value: mrpDet.officeLocId, label: mrpDet.offLocName, bNo: mrpDet.branchCode },
        departId: { value: mrpDet.departId, label: mrpDet.deptName },
        divId: { value: mrpDet.divId, label: mrpDet.divName },
        purType: { value: mrpDet.purType, label: mrpDet.purType },
        prodDet: mrpDet.itemDTOs,
      }));
      setMrpDetails(mrpDetData);
    }catch(error){
      console.error('Error fetching Saved MR Plan Details: ', error);
      toast.error('Failed to fetch Saved MR Plan Details.')
    }finally{
        setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    await axios.post(`${BASE_URL_P2P}mrPlan/saveSubmit/${id}`, payLoad, {
      headers: {
        Authorization: `Bearer ${jwtToken}`,
        "Content-Type": 'application/json',
      }
  ...
  const handleSave = async (e) => {
    ...
    if(addedProducts.length === 0){
      ...
      setLoading(true);
      await axios.post(`${BASE_URL_P2P}mrPlan/saveAgain/${id}`, payLoad, {
        headers: {
          Authorization: `Bearer ${jwtToken}`,
          "Content-Type": 'application/json',
        }
  ...
  useEffect(() => {
    if(mrpDetails.length > 0){
      const mrpDet = mrpDetails[0];
      setLocId(mrpDet.officeLocId);
      setDeptId(mrpDet.departId);
      setDivId(mrpDet.divId);
      setPurchaseType(mrpDet.purType);
      const products = mrpDet.prodDet.map((item) => ({
        itemId: {
      ...
      <CCard>
        <CCardTitle>
          <CRow className='d-flex justify-content-between align-items-center'>
            <CCol>Saved MR Plan Request</CCol>
          ...
          <CCardBody className='pt-1'>
            <CRow>
              <CCol xs={12} md={toggleSummary && activeTab !== 2 ? 9 : 12}>
                <CRow>
                  <CCol xs={12} sm={3} className='border-r border-r-gray-300 min-h-[68vh]'>
