...
const PurchaseType_DP = ({ typeRef, purchaseTypes, purchaseType, setPurchaseTypes, setPurchaseType, categoryId, setCategoryId }) => {
  ...
  return (
    <div className="floating-label">
      <Select id='Purchase-Type-select' value={purchaseTypes.find(pur => pur.value === purchaseType)} onChange={(e) => { setPurchaseType(e); categoryId && setCategoryId(''); }} onFocus={e => e.target.classList.add('focused')} onBlur={(e) => !purchaseType && e.target.classList.remove('focused')} options={purchaseTypes} placeholder="" ref={typeRef} isMulti={false} isSearchable />
      <label htmlFor='Purchase-Type-select' className={purchaseType ? 'active' : 'floating'}>Purchase Type</label>
    </div>
  )
}

export default PurchaseType_DP